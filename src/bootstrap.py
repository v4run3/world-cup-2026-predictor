...
def ensure_ready(log=print) -> None:
    ...
    # 2. Processed features / Elo / team state.
    if _need(config.FEATURES_CSV, config.ELO_RATINGS, config.TEAM_STATE_CSV):
        log("Building features and Elo ratings...")
        from src.data_collection import load_results
        from src.features import build_features, current_elo_ratings, current_team_state
